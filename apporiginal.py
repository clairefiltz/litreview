...
file_path = 'https://storage.googleapis.com/wagon-data-735-vianadeabreu/data/trimmed_arxiv_docs5000.csv'
...
if st.button('click me'):
    ...
    if title in list(test_df['title']):
        papers, vectorizer, model = search.run_search()
        st.write('Related papers:', papers[title])
        st.write(f'Vectorizer with: {vectorizer}')
        st.write(f'Model with: {model}')
    else:
