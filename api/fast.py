...
def predict(user_input, neighbors=3):
    ...
    for i in indices:
        ...
        query = f"""SELECT index,id,title,abstract,authors,versions
        ...
        id = data.loc[0, 'id']
        author = data.loc[0, 'authors']
        version = data.loc[0, 'versions']
        result[pos_neigh] = [title, abstract, id, author, version]
        pos_neigh += 1
    end_time = time.time()
